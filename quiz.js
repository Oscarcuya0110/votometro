...
// ------------------------------------------------------------------
let userProfile   = {};
let answers       = new Array(22).fill(null);
let currentQ      = 0;
...
/**
 * showScreen(id) — transición fluida entre pantallas usando CSS transitions.
 *
 * Flujo:
 *   1. La pantalla actual recibe .leaving → dispara fade out CSS (180ms)
 *   2. La pantalla nueva recibe .active   → dispara fade in CSS (350ms)
 *   3. Tras 180ms removemos .leaving de la antigua para limpiar el DOM
 *
 * Importante: usamos requestAnimationFrame para garantizar que el browser
 * haya pintado el estado inicial antes de aplicar la transición de entrada,
 * evitando el "flash" que ocurre cuando opacity:0 → opacity:1 pasan en el
 * mismo frame sin que el motor de CSS haya registrado el estado de origen.
 */
function showScreen(id) {
  ...
  if (current) {
    // Paso 1: fade out de la pantalla actual
    current.classList.add('leaving');
    current.classList.remove('active');
  }

  // Paso 2: aseguramos que el browser haya procesado el estado opacity:0
  // de la pantalla nueva ANTES de aplicar .active, para que la transición
  // de entrada sea visible y no instantánea.
  ...
  // Paso 3: limpieza — removemos .leaving cuando la animación de salida terminó
  if (current) {
    setTimeout(() => {
      current.classList.remove('leaving');
    }, 320); // un poco más largo que la duración CSS de .leaving (0.18s → 180ms)
  }
}
 ...
 */
function goHome() {
  answers     = new Array(22).fill(null);
  currentQ    = 0;
  userProfile = {};
